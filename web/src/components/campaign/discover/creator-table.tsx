...
export function CreatorTable({
  ...
  function renderCell(col: (typeof ALL_COLUMNS)[number], c: CreatorWithStatus) {
    ...
    switch (col.id) {
      ...
      case "followers":
        return <TableCell key={col.id} className={cellClass}>{formatNumber(c.followers)}</TableCell>
      case "avg_views":
        return <TableCell key={col.id} className={cellClass}>{formatNumber(c.avg_views)}</TableCell>
      case "engagement_rate":
        return <TableCell key={col.id} className={cellClass}>{c.engagement_rate ? `${(c.engagement_rate * 100).toFixed(1)}%` : "\u2014"}</TableCell>
      case "total_likes":
        return <TableCell key={col.id} className={cellClass}>{formatNumber(c.total_likes)}</TableCell>
      case "video_count":
        return <TableCell key={col.id} className={cellClass}>{c.video_count || "\u2014"}</TableCell>
      case "emails":
        return (
